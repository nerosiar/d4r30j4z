...
static void toggle_layer (GtkToggleButton *widget,ClutterActor *layer)
{
...
static gboolean mouse_click_cb (ClutterActor *actor, ClutterButtonEvent *event, ChamplainView *view)
{
  gdouble lat, lon;
...
static void map_source_changed (GtkWidget *widget, ChamplainView *view)
{
  gchar *id;
...
static void zoom_changed (GtkSpinButton *spinbutton,ChamplainView *view)
{
...
static void view_state_changed (ChamplainView *view,GParamSpec *gobject,GtkImage *image)
{
...
static void zoom_in (GtkWidget *widget, ChamplainView *view)
{
...
static void build_combo_box (GtkComboBox *box)
{
  ...
  gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (box), cell,"text", COL_NAME, NULL);
}
...
static void add_clicked (GtkButton     *button,ChamplainView *view)
{
...
int main (int argc,char *argv[])
{
  ...
  /* give it the title */
  gtk_window_set_title (GTK_WINDOW (window), "Position");
  ...
  // zoom initial
  g_object_set (G_OBJECT (view),"kinetic-mode", TRUE,"zoom-level", 18,NULL);
  ...
  champlain_license_set_extra_text (license_actor, "GeekGods \n all rights reserved");
  //maison de retraite
  champlain_view_center_on (CHAMPLAIN_VIEW (view), 36.898392, 10.187538);
